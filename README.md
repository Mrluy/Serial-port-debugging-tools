...
- 发送区和接收区的输入控件更紧凑，减少工具行占用的垂直空间。
- 支持多个连接同时打开，顶部会话标签可并排切换并关闭。
- 发送内容区使用中文右键菜单；接收区按列显示记录，并支持拖选任意文本片段后右键复制。
- 顶部标签只显示已打开过的连接目标，关闭标签只断开连接，不删除连接列表中的目标；每个标签保留独立的发送内容和接收记录。
- 接收区表头和数据列按比例自适应窗口宽度，始终占满整行。
- 接收区始终预留右侧滚动条宽度，避免数据增多出现滚动条时列宽跳动。
- 顶部标签页关闭按钮采用 macOS 风格圆点加 `×`。
