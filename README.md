...
- 支持暂停显示、保存接收区、实时保存到文件
- 发送区显示发送历史，并按时间、连接、数据分列显示
- 接收区按时间、连接、数据分列显示
- 发送区和接收区默认固定显示时间和连接信息
- 底部状态栏显示当前连接状态、连接名称、收发计数和收发速度
- 发送区支持右键复制、剪切、粘贴，接收区支持右键复制
- 显示发送/接收字节计数和每秒收发速度
- 使用 `assets/app.png` 作为窗口和 EXE 图标

## 界面布局

`v1.0.6` 起默认使用深蓝黑暗色主题。主窗口左侧是连接列表、串口或网络参数、计数区；右侧是当前会话标签页、发送区和接收区。
...
如果原来使用文本模式发送，程序会把追加 CRC 后的完整数据转换为 16 进制并写回发送区，同时切换到 `16进制` 发送模式，避免 CRC 字节被当作普通文本处理。
